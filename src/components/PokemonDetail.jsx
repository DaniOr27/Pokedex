    ...
    return (
        <>
            <header className="absolute left-1/2 -translate-x-1/2 -translate-y-[92%] scale-[180%]">
                ...
                <ul 
                className="flex gap-2 justify-center capitalize">
                    {pokemon?.types.map(type => (
                        <li 
                            className={`p-1 rounded-md px-2 text-gray-100 text-sm ${colorByType[type]}`} 
                            key={type}>
                            {type}
                        </li>                  
                    ))}
                    ...
                    <ul className="grid grid-cols-2 gap-4">
                        {
                            pokemon?.abilities.map((ability) => (
                                <li 
                                key={ability} className="bg-slate-100 block rounded-full p-1 capitalize">
                            <span> {ability} </span>
                        </li>
                            ))
                        }
